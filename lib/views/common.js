var Backbone, Common, exports, _;

_ = require('lodash');

Backbone = require('backbone');

Common = Backbone.Model.extend({
  common: function() {
    this.set('pretty', true);
    this.set('css', []);
    this.set('js', []);
    this.set('scripts', []);
    this.set('globalObjects', []);
    return this.set('json_locale_data', {
      messages: {}
    });
  },
  addLocalCssFile: function(file) {
    var ms;
    if (file == null) {
      file = 'theme';
    }
    ms = this.get('css');
    file = this.editLocalFileName(file, 'css');
    ms.push(file);
    return this.set('css', ms);
  },
  addLocalJsFile: function(file) {
    var ms;
    if (file == null) {
      file = 'sn';
    }
    ms = this.get('js');
    file = this.editLocalFileName(file, 'js');
    ms.push(file);
    return this.set('js', ms);
  },
  addScript: function(script) {
    var ms;
    if (script == null) {
      script = '';
    }
    ms = this.get('scripts');
    ms.push(script);
    return this.set('scripts', ms);
  },
  exportGlobalObject: function(name, obj) {
    var ms;
    if (name == null) {
      name = 'val';
    }
    if (obj == null) {
      obj = {};
    }
    ms = this.get('globalObjects');
    ms.push({
      name: name,
      data: obj
    });
    return this.set('globalObjects', ms);
  },
  editLocalFileName: function(file, ext) {
    var version;
    if (file == null) {
      file = '';
    }
    if (ext == null) {
      ext = 'css';
    }
    version = this.get('version');
    if (!file.match(/\//)) {
      file = file.replace(/^\_/, '');
      file = file.replace(/.css$/, '');
      file = file.replace(/.less$/, '');
      file = file.replace(/.js$/, '');
      file = file.replace(/.lmd$/, '');
      file = file.replace(version, '');
      file = file.replace(/\.\./, '.');
      if (ext === 'css') {
        file = "" + (this.get('theme')) + "/_" + file + "." + version + "." + ext;
      }
      if (ext === 'js') {
        file = "" + file + ".lmd." + version + "." + ext;
      }
    }
    return file;
  },
  addLocaleString: function(keys) {
    var key, ms, req, _i, _len;
    if (typeof keys === 'string') {
      keys = _.toArray(arguments);
    }
    req = this.get('req');
    ms = this.get('json_locale_data');
    for (_i = 0, _len = keys.length; _i < _len; _i++) {
      key = keys[_i];
      ms['messages'][key] = [null, req.gettext(key)];
    }
    return this.set('json_locale_data', ms);
  }
});

exports = module.exports = Common;
