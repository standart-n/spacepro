...
User.prototype.getUserList = function(fn) {
  ...
  this.fbCreateConnection(function(err, db) {
    var sql = _this.sqlUserList();
    if (!err) {
      db.query(sql, function(err, result) {
        debug('User.prototype.getUserList'.magenta, sql.yellow);
        db.detach();
        if (!err) {
          debug('User.prototype.getUserList'.magenta, result.length);
          _this.set('users', result);
          fn(null);
        } else {
          debug('User.prototype.getUserList'.magenta, err);
          fn(err);
        }
      });
    } else {
...
User.prototype.getUserGroups = function(fn) {
  ...
  this.fbCreateConnection(function(err, db) {
    var sql = _this.sqlUserGroups();
    if (!err) {
      db.query(sql, function(err, result) {
        debug('User.prototype.getUserGroups'.magenta, sql.yellow);
        db.detach();
        if (!err) {
          debug('User.prototype.getUserGroups'.magenta, result.length);
          _this.set('groups', result);
          fn();
        } else {
          debug('User.prototype.getUserGroups'.magenta, err);
          fn(err);
        }
      });
    } else {
...
User.prototype.updatePasswordByUserLogin = function(userlogin, userpassword, fn) {
  ...
  var hash = md5(userpassword).toString().toUpperCase();
