...
var _, md5, colors, moment, mongoose, async, bar, Privilege, Firebird, User, UserSchema, UserModel;
...
User.prototype.updateAllUsers = function(fn) {
  ...
  this.fbTransactionOpen(function() {
    ...
    async.parallel({
      ...
      listOfDicts: function(fn) {
        bar.getListOfSids(fn);
      }

    }, function(err, results) {
      if ((!_this.get('error')) && (!_this.get('fb_error')) && (!err)) {
        _this.set('dicts', results.listOfDicts || []);
...
User.prototype.updateCacheOfUsers = function(fn) {
  ...
  _.each(users, function(user) {
    userFn.push(function(fn) {
      UserModel.findOne({
        ...
        privilege.getPrivileges(function(err, result) {
  ...
  async.parallel(userFn, function() {
    fn();
  });
};
