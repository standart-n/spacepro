...
Grid.prototype.copyGridByUserLogin = function(userlogin, fn) {
  var _this = this;
  ...
  this.fbCreateConnection(function(err, db) {
    if (!err) {
      db.query(_this.sqlGetByUserLogin(userlogin), function(err, result) {
        if (!err) {
          if (result[0].userdata != null) {
            db.query(_this.sqlUpdateGridByUserId(), [result[0].userdata], function(err, result) {
              db.detach();
              if (!err) {
                fn(null, 'success');
              } else {
...
Grid.prototype.sqlGetByUserLogin = function(userlogin) {
  return "select \n" +
         "* \n" +
         "from sp$users \n" + 
         "where \n" +
...
Grid.prototype.sqlUpdateGridByUserId = function() {
  return "update  \n" +
         "sp$users \n" +
         "set userdata = ? \n" +
         "where \n" +
