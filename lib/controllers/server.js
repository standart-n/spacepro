var Server, async, cluster, colors, exports, express, http, mkpath, mongoose, readline, store, _;
...
Server = (function() {
  ...
  Server.prototype.configure = function(special, handler) {
    ...
    return async.series([
      function(fn) {
        return _this.answer(store, 'port', fn);
      }, function(fn) {
        return _this.answer(store, 'theme', fn);
      }, function(fn) {
        return _this.answer(store, 'firebird_host', fn);
      }, function(fn) {
        return _this.answer(store, 'firebird_path', fn);
      }, function(fn) {
        return _this.answer(store, 'firebird_user', fn);
      }, function(fn) {
        return _this.answer(store, 'firebird_password', fn);
      }, function(fn) {
        return _this.answer(store, 'mongodb_connection', fn);
      }
  ...
  Server.prototype.answer = function(store, key, fn) {
    ...
    if ((this.options[key] == null) && !store.get(key) && !proc(key)) {
      ...
      return store.question(this.rl, key, function(value) {
        return fn(null, value);
      });
    } else {
      if (!proc(key)) {
        if (!store.get(key)) {
          return fn(null, this.options[key]);
        } else {
          return fn(null, store.get(key));
        }
      } else {
        return fn(null, proc(key));
      }
    }
  };
  ...
  Server.prototype.init = function(results) {
    ...
    return process.env.MONGODB_CONNECTION = results[6];
  };
  ...
  Server.prototype.run = function(special, handler) {
    ...
    return this.configure(special, function() {
      var app, server;
      app = express();
