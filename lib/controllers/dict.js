...
var _, async, ini, str, mongoose, md5, Grid, Firebird, Dict, Privilege, Sqlmaster, DictSchema, DictModel;
...
Dict.prototype.getPrivileges = function(fn) {
  ...
  privilege.getPrivilegesBySidAndUserId(function(err, privileges) {
    if (err) {
      fn(err);
    } else {
