...
var _, async, ini, str, fs, mongoose, md5, Grid, Firebird, Dict, User, Privilege, Sqlmaster, DictSchema, DictModel;
...
Dict.prototype.getHeaderInfoOfDict = function() {
  ...
  return {
    ...
    'returnfieldname':     this.get('returnfieldname'),
    'captionfieldname':    this.get('captionfieldname'),
    'keyfieldname':        this.get('keyfieldname'),
    'renderitemsearch':    this.get('renderitemsearch'),
    'renderoptionsearch':  this.get('renderoptionsearch'),
    'privileges':          this.get('privileges'),
    'toolbar':             this.get('toolbar'),
    'cfselect':            this.get('cfselect'),
    'addfields':           this.get('addfields'),
    'editfields':          this.get('editfields'),
    'renderItemSearch':    this.get('renderItemSearch'),
    'renderOptionSearch':  this.get('renderOptionSearch'),
    'childsInfo':          this.get('childsInfo'),
    'selectRowUUID':       this.get('selectRowUUID'),
...
Dict.prototype.getMainInfoOfDict = function() {
  ...
  _.each(this.get('depDicts'), function(depDict) {
    depDicts.push(depDict.getHeaderInfoOfDict());
  });

  data = this.getHeaderInfoOfDict();
  data.childs =    childs;
  data.depDicts =  depDicts;
...
Dict.prototype.initBySid = function(fn) {
  ...
  _this.fbTransactionOpen(function() {
    if (_this.get('fb_transaction')) {
      ...
      // console.log(_this.sqlGetByDictSid());
...
Dict.prototype.getDictInfo = function(result, fn) {
  ...
  if (typeof result.ini === 'string') {
    ...
    fs.writeFile(process.env.APP_DIR + '/dicts/' + result.sid.trim() + '.ini', result.ini);
  ...
  if (typeof result.ini === 'string') {
    this.updateCacheOfDictSettings(fn);
  } else {
    fn();
  }
};
...
Dict.prototype.insertLine = function(fn) {
  ...
  execute = function(fn) {
    if (_this.get('fb_transaction')) {
      tr = _this.get('fb_transaction');
      _this.executePreSql(controls, function(controls) {
