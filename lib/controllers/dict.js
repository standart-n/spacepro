...
Dict = Firebird.extend({
  ...
  getMainInfoOfDict: function() {
    var data, 
      childs = [],
      i = 0;

    _.each(this.get('childs'), function(child) {
      childs.push(child.getHeaderInfoOfDidct());
    });

    data = this.getHeaderInfoOfDidct();
    data.childs = childs;
