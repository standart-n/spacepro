...
Dict.prototype.getDictInfo = function(result, fn) {
  var settings, main, path, tr, file, sid,
    _this = this;
  ...
  sid = result.sid.toString().trim().toLowerCase();
  ...
  if (typeof result.ini === 'string') {
    ...
    if (this.get('upgrade') === true) {
      ...
      this.fbTransactionOpen(function() {
        if (_this.get('fb_transaction')) {
          ...
          tr.query(_this.sqlUpdateIniBySid(), [file], function(err, result) {
            if (err) {
              _this.fbCheckError('Not upgrade dict ' + sid, fn);
            } else {
              _this.fbTransactionCommit();
            } 
          });
      ...
      console.log(iconv.decode(new Buffer(file), 'utf8'));
