...
Dict.prototype.editLine = function(fn) {
  ...
  // console.log('mmbshgroup', this.get('mmbshgroup'));
  // console.log('groupfield', this.get('groupfield'));
  ...
  if (field !== 'mmbsh') {
    ...
    this.fbCreateConnection(function(err, db) {
      var sql;
      if (!err) {
        sql = _this.sqlUpdate(sourcetablename, keyfieldname, key, field, value);
                ...
                function(fn) {
                  if (value.add) {
                    async.each(value.add, function(group, fn) {
                      var sql, table;
                      table = sourcetablename.toUpperCase();
                      sql =   _this.sqlAddGroup(group, groupkey, mmbshgroup);
                ...
                function(fn) {
                  if (value.del) {
                    async.each(value.del, function(group, fn) {
                      var sql, table;
                      table = sourcetablename.toUpperCase();
                      sql =   _this.sqlDeleteGroup(group, groupkey, mmbshgroup);
                      tr.query(sql, [group, groupkey, mmbshgroup, 0], function(err) {
                        if (!err) {
                          fn();
                        } else {
...
Dict.prototype.sqlUpdate = function(table, keyfield, key, updatefield, value) {
  var sqlmaster = this.get('sqlmaster') || new Sqlmaster();
  return sqlmaster.update(table, keyfield, key, updatefield, value);
};
