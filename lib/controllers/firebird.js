var Backbone, exports, fb, _;

_ = require('lodash');

Backbone = require('backbone');

fb = require('node-firebird');

exports = module.exports = Backbone.Model.extend({
  defaults: function() {
    return {
      fb_host: process.env.FIREBIRD_HOST,
      fb_database: process.env.FIREBIRD_PATH,
      fb_user: process.env.FIREBIRD_USER,
      fb_password: process.env.FIREBIRD_PASSWORD,
      fb_connection: null,
      fb_transaction: null,
      error: null,
      hide_errors: null
    };
  },
  initialize: function() {},
  fbConnectionCreate: function(fn) {
    var _this = this;
    if (fn == null) {
      fn = function() {};
    }
    if (!this.get('fb_host')) {
      this.set('fb_host', process.env.FIREBIRD_HOST);
    }
    if (!this.get('fb_database')) {
      this.set('fb_database', process.env.FIREBIRD_PATH);
    }
    if (!this.get('fb_user')) {
      this.set('fb_user', process.env.FIREBIRD_USER);
    }
    if (!this.get('fb_password')) {
      this.set('fb_password', process.env.FIREBIRD_PASSWORD);
    }
    return fb.attach({
      host: this.get('fb_host'),
      database: this.get('fb_database'),
      user: this.get('fb_user'),
      password: this.get('fb_password')
    }, function(err, connect) {
      return fn(err, connect);
    });
  },
  fbConnectionOpen: function(fn) {
    var _this = this;
    if (fn == null) {
      fn = function() {};
    }
    if (!this.get('error')) {
      if (!this.get('fb_connection')) {
        return this.fbConnectionCreate(function(err, connect) {
          if (connect != null) {
            _this.set('fb_connection', connect);
            return fn(null, connect);
          } else {
            return _this.fbCheckError(err, fn);
          }
        });
      } else {
        return fn(null, this.get('fb_connection'));
      }
    } else {
      return fn();
    }
  },
  fbConnectionClose: function() {
    var connect;
    if (this.get('fb_connection')) {
      connect = this.get('fb_connection');
      connect.detach();
      return this.unset('fb_connection');
    }
  },
  fbTransactionStart: function(fn) {
    var connect,
      _this = this;
    if (fn == null) {
      fn = function() {};
    }
    if (this.get('fb_connection')) {
      connect = this.get('fb_connection');
      return connect.startTransaction(function(err, tr) {
        if (tr != null) {
          _this.set('fb_transaction', tr);
          return fn(null, tr);
        } else {
          return _this.fbCheckError(err, fn);
        }
      });
    } else {
      return this.fbCheckError('Connection not found', fn);
    }
  },
  fbTransactionCommit: function(tr, fn) {
    var _this = this;
    if (fn == null) {
      fn = function() {};
    }
    if (tr != null) {
      return tr.commit(function(err) {
        _this.unset('fb_transaction');
        if (!_this.fbCheckError(err, fn)) {
          return fn(null, 'commit');
        }
      });
    }
  },
  fbTransactionRollback: function() {
    var tr;
    if (this.get('fb_transaction')) {
      tr = this.get('fb_transaction');
      tr.rollback();
      return this.unset('fb_transaction');
    }
  },
  fbCheckError: function(err, fn) {
    var req;
    if (fn == null) {
      fn = function() {};
    }
    if (err != null) {
      req = this.get('req') ? this.get('req') : {
        gettext: function(s) {
          return s;
        }
      };
      if (typeof err === 'object') {
        err = JSON.stringify(err);
      }
      if (typeof err === 'function') {
        err = err();
      }
      err = req.gettext(err);
      if (!this.get('error')) {
        this.set('error', err);
        if (this.get('hide_errors') !== true) {
          console.error(err.red);
        }
      }
      this.fbTransactionRollback();
      fn(err);
      return true;
    } else {
      return false;
    }
  }
});
