...
var Firebird, Session, Auth, Validate, async, colors, middleware, md5, Test;
...
middleware = function(options) {
  ...
  return function(req, res, next) {
    ...
    if (req.session != null) {
      ...
      if ((req.session.fbuser.id != null) && (req.session.workstation.id != null) && (req.session.fbsession.id != null)) {

        memcached = global.memcached;
        hash = md5(JSON.stringify(req.session.fbuser) + JSON.stringify(req.session.workstation) + JSON.stringify(req.session.fbsession));

        memcached.get(hash, function(err, result) {
          if (!result) {
            auth = new Auth({
              req:              req,
              user_id:          req.session.fbuser.id,
